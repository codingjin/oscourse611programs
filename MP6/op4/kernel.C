...
void fun1() {
    ...
    for(int j = 0;; j++) {
       ...
       Console::puts("FUN 1 IN ITERATION["); Console::puti(j); Console::puts("]\n");
	    /*
		Console::puts("Fun1 will enter CS\n");
		lock(&ll);
		Console::puts("Fun1 enters CS\n");
		pass_on_CPU(thread2);
		for (int k=0;k<9999;++k);
		Console::puts("Fun1 finishes CS\n");
		unlock(&ll);
		*/
...
void fun2() {
    ...
    for(int j = 0;; j++) {
       ...
       //Console::puts("fun2 finish Reading a block from disk...\n");
       ...
       //Console::puts("fun2 finish Writing a block to disk...\n");
...
void fun3() {
	...
	*/
    ...
    for(int j = 0;; j++) {
       ...
       SYSTEM_DISK1->read(read_block, buf);
       //Console::puts("fun3 finish Reading a block from disk...\n");
       ...
       SYSTEM_DISK1->write(write_block, buf); 
       //Console::puts("fun3 finish Writing a block to disk...\n");
...
void fun4() {
    ...
    for(int j = 0;; j++) {
	   ...
	   /*
	   Console::puts("Fun4 will enter CS\n");
		lock(&ll);
		Console::puts("Fun4 enters CS\n");
		for (int k=0;k<9999;++k);
		Console::puts("Fun4 finishes CS\n");
		unlock(&ll);
		*/
...
int main() {
	...
	// Initialize block_mutex
	//for (int i=0;i<DISK_BLOCK_NUM;++i)	block_mutex[i]=0;
    ...
    SimpleTimer timer(10); /* timer ticks every 10ms. */
    InterruptHandler::register_handler(0, &timer);
    //InterruptHandler::register_handler(14, &timer);
	...
	// -- Lock Initialization --
	// DISK_BLOCK_NUM and mutex are defined in ts_disk.H
	//for (int i=0;i<DISK_BLOCK_NUM;++i)	block_mutex[i]=0;

    /* -- DISK DEVICE -- */

    //SYSTEM_DISK = new SimpleDisk(MASTER, SYSTEM_DISK_SIZE);
    SYSTEM_DISK = new TSDisk(MASTER, SYSTEM_DISK_SIZE);
    SYSTEM_DISK1 = new TSDisk(MASTER, SYSTEM_DISK_SIZE);
