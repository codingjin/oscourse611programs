...
void exercise_file_system(FileSystem * _file_system) {
	...
	Console::puts("CreateFile passes!\n");
    ...
    assert(file2 != NULL);
	Console::puts("LookupFile passes!\n");
	...
	Console::puts("Rewrite and Write pass!\n");
...
int main() {
	...
	/*
	Console::puts("size of int=");
	Console::puti(sizeof(int));
	Console::puts("\n");

	unsigned char bb[128];
	for (int i=0;i<128;++i)	bb[i]=0;
	int A = 1136;
	bb[0] = (unsigned char)(A & 0xFF);
	bb[1] = (unsigned char)((A>>8) & 0xFF);
	bb[2] = (unsigned char)((A>>16) & 0xFF);
	bb[3] = (unsigned char)((A>>24) & 0xFF);

	int B = bb[0] + (bb[1]<<8) + (bb[2]<<16) + (bb[3]<<24);
	...
	*/
    ...
    char * stack3 = new char[10240];
    thread3 = new Thread(fun3, stack3, 10240);
    Console::puts("DONE\n");
