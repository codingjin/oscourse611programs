...
/* UNCOMMENT THE FOLLOWING LINE IF YOU WANT A DUMMY THREAD TO RUN */
//#define _DUMMYTHREAD_
...
#ifdef _DUMMYTHREAD_
void fun_dummy() {
	Console::puts("Thread Dummy\n");
	for(;;) {
		Console::puts("Dummy ticks\n");
	}
}
#endif
...
int main() {
    ...
    //SimpleTimer timer(2); /* timer ticks every 10ms. */
...
#ifdef _DUMMYTHREAD_
	Console::puts("CREATING THREAD_DUMMY ...");
	char * stack_dummy = new char[1024];
	thread_dummy = new Thread(fun_dummy, stack_dummy, 1024);
	Console::puts("Done\n");
#endif
	...
	//Console::putui((unsigned int)thread1);
	//Console::putui((unsigned int)thread2);
	//Console::putui((unsigned int)thread3);
	//Console::putui((unsigned int)thread4);
#endif

#ifdef _DUMMYTHREAD_
	SYSTEM_SCHEDULER->add(thread_dummy);
#endif
